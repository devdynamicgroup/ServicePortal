...
console.log('\nFIXED — Thailand TDS / turbidity / chlorine in-band severity');
{
  assert(grade('thailand', 'tds', 300) === 100, 'TH TDS ≤300 = 100');
  assert(grade('thailand', 'tds', 800) < 100 && grade('thailand', 'tds', 800) > grade('thailand', 'tds', 1000),
    'TH TDS 800 < 100 and better than 1000');
  assert(grade('thailand', 'turbidity', 1) === 100, 'TH turb ≤1 = 100');
  ...
  assert(bench('thailand', DIFF).score === 87, `DIFF TH still 87 (got ${bench('thailand', DIFF).score})`);
}

console.log('\nNOT FIXED (governance) — pH flat-in-band Compliance Index (PD-006)');
{
...
console.log('\nNOT FIXED (governance) — ORP flat 200–600 (PD-004)');
{
  for (const key of KEYS) {
    assert(grade(key, 'orp', 200) === 100, `${key} ORP 200 = 100`);
    assert(grade(key, 'orp', 350) === 100, `${key} ORP 350 = 100`);
    assert(grade(key, 'orp', 600) === 100, `${key} ORP 600 = 100`);
    assert(grade(key, 'orp', 100) < 100, `${key} ORP 100 < 100`);
    assert(grade(key, 'orp', 700) < 100, `${key} ORP 700 < 100`);
  }
}

console.log('\nNOT FIXED (governance) — EPA Cl flat projectMin..MRDL (PD-008)');
{
  for (const cl of [0.2, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0]) {
    assert(grade('usEpa', 'chlorine', cl) === 100, `EPA Cl ${cl} = 100 (compliance window)`);
  }
  assert(grade('usEpa', 'chlorine', 0.1) < 100, 'EPA Cl 0.1 < 100');
  assert(grade('usEpa', 'chlorine', 4.01) < 100, 'EPA Cl > MRDL < 100');
}
...
console.log('\nRAW vs engine input + Hero path (DIFF)');
{
  ...
  assert(th.score === 87 && cmp.score === 87 && disp.score === 87,
    'engine === comparison === displayed = 87');
...
console.log('\nCross-country freeze on standard fixtures (non-TH engines)');
{
  ...
  assert(sandbox.computeQualityScoreDetail(BASE).score === 76, 'Q-V3 BASE 76');
  assert(bench('japan', DIFF).score === 78, 'JP DIFF 78');
  assert(bench('eu', DIFF).score === 61, 'EU DIFF 61');
  assert(bench('who', DIFF).score === 81, 'WHO DIFF 81');
  assert(bench('usEpa', DIFF).score === 79, 'EPA DIFF 79');
  assert(bench('japan', LOCKED).score === 96, 'JP LOCKED 96');
  assert(bench('thailand', LOCKED).score === 95, 'TH LOCKED 95');
}
