...
# Stable fallbacks matching Phase 2/3.5 mock API contract
...
def merge_with_fallback(meter_type: str, data: dict[str, Any] | None) -> dict[str, Any]:
    kind = (meter_type or "tds").lower()
    base = dict(_FALLBACK_DATA.get(kind, _FALLBACK_DATA["tds"]))
    if data:
