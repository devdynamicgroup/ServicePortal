...
function run(env, code) {
  ...
  ['AUTH_SESSION_SECRET', 'SESSION_SECRET', 'AUTH_USERS_JSON', 'AUTH_ALLOW_DEV_USERS', 'RENDER', 'RENDER_SERVICE_ID', 'NODE_ENV'].forEach(key => {
...
const missing = run(
  { NODE_ENV: 'production', AUTH_SESSION_SECRET: '', SESSION_SECRET: '', AUTH_USERS_JSON: USERS_JSON, RENDER: '', RENDER_SERVICE_ID: '' },
...
const unsetUsers = run(
  { NODE_ENV: 'production', AUTH_SESSION_SECRET: 'unit-test-secret-value-32chars!!', AUTH_USERS_JSON: '', AUTH_ALLOW_DEV_USERS: '', RENDER: '', RENDER_SERVICE_ID: '' },
  'require("./services/app-auth")'
);
console.log('prod_missing_users_exit', unsetUsers.status);
console.log('prod_missing_users_has_fatal', /AUTH_USERS_JSON/.test(String(unsetUsers.stderr || '') + String(unsetUsers.stdout || '')));

const unsetNode = run(
  { NODE_ENV: '', AUTH_SESSION_SECRET: '', SESSION_SECRET: '', AUTH_USERS_JSON: USERS_JSON, RENDER: '', RENDER_SERVICE_ID: '' },
...
const dev = run(
  { NODE_ENV: 'development', AUTH_SESSION_SECRET: '', SESSION_SECRET: '', AUTH_USERS_JSON: USERS_JSON, RENDER: '', RENDER_SERVICE_ID: '' },
  'require("./services/app-auth"); console.log("dev_ok")'
);
console.log('dev_exit', dev.status, (dev.stdout || '').trim().split('\n').pop());

const prodOk = run(
  { NODE_ENV: 'production', AUTH_SESSION_SECRET: 'unit-test-secret-value-32chars!!', AUTH_USERS_JSON: USERS_JSON, RENDER: '', RENDER_SERVICE_ID: '' },
